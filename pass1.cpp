...
void pass1()
{
  ...
  if(opcode=="START")
  {
    ...
    getline(sourceFile,fileLine); //eead next line
    lineNumber += 5;
    index = 0;
    ...
    if(!checkCommentLine(fileLine))
    {
      ...
      else if(opcode=="ORG")
      {
        ...
        if(SYMTAB[operand].exists=='y'){
          ...
          bool relative;
          error_flag = false; //set error_flag to false
      ...
      else if(opcode=="EQU")
      {
        ...
        else if(if_all_num(operand))
        {
          ...
          while(lastByte=='+'||lastByte=='-'||lastByte=='/'||lastByte=='*') //code for reading whole operand
          {
            readFirstNonWhiteSpace(fileLine,index,statusCode,tempOperand);
            operand += tempOperand;
            lastByte = operand[operand.length()-1];
