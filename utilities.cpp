...
string intToStringHex(int x,int fill = 5) //  convert integer to (string)hexadecimal format
{
  stringstream s;
...
string expandString(string data,int length,char fillChar,bool back=false)
{
  if(back)
  {
    if(length<=data.length()) return data.substr(0,length);
    else
    {
  ...
  {
    if(length<=data.length()) return data.substr(data.length()-length,length);
    else
    {
      for(int i = length-data.length();i>0;i--)
      {
...
void readByteOperand(string line,int& index,bool& status,string& data)
{
  ...
  {
    data += line[index++];
    char identify = line[index++];
    data += identify;
    while(index<line.length() && line[index]!=identify) //Copy all data until next identify regardless of whitespace
    { 
      data += line[index];
      index++;
    }
    data += identify;
    index++;//Shift to next of identify
  }
  else{
