...
string createObjectCodeFormat34()
{
  ...
  {
    string tempOperand = operand.substr(1,operand.length()-1);
    if(tempOperand.substr(tempOperand.length()-2,2)==",X" || SYMTAB[tempOperand].exists=='n') //rerror handling for Indirect with index based
    {
      writeData = "Line "+to_string(lineNumber);
...
void pass2()
{
  ...
  getline(intermediateFile, tempBuffer); // discard to heading line
